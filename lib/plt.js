/*
 ** this file is modified from original for comicstripscript by Yuki Yoshida **
 *
 * plt.js 0.2.0
 *
...
PLT.fn.stringifiedParse = function(source) {
	return JSON.stringify(PLT.parser.parse(source));
};

PLT.fn.setGrammarFromTag = function(tagname){
	// extract PEG grammar from <grammar> element and build parser
	var grammarElement = $(tagname);
//	console.log(grammarElement.text());
...
PLT.fn.buildRepl = function(){
	...
	var startPrompt = function () {
		repl.Prompt(true, function (input) {
			try {
				var evalfn = PLT.repl || PLT.fn.stringifiedParse;
	...
	for (var i = 0; i < goods.length; i++) {
		try{
			...
			if(err.line == goods[i].textContent.split('\n').length){
				this.carrot = Array(err.column).join(' ')+'&uarr;\n'
			}
...
PLT.fn.parseCodesFromString = function(string){
	try {
		var str = PLT.fn.stringifiedParse(string);
		//---the code is parsed, print result in console
		//console.log(str);
	} catch (err) {
		//---the code is not parsed, print error in console
		//---If there is line info, add information about where the error is
		var lineError = '';
		if(err.line){
			lineError = "<br>\t" + "Line " + err.line + " Column " + err.column;
		}
		console.log(this.carrot + " " + err.message + lineError);
	}
}
...
$(function() {
	...
	PLT.fn.injectCss();
	PLT.fn.setGrammarFromTag("grammar");

//	PLT.fn.buildRepl();
//	$('<h2>' + $('title').text() + '</h2>').prependTo("body");
//	PLT.fn.parseCodesFromTag();
